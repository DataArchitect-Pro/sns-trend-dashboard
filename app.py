...
# Sランク条件 (新規性と成長があり、最低限の継続性を満たし、今すぐ仕込む価値がある)
...
def enrich_card_data(row):
    ...
    # 【パターン2系・4系】A（保留）: 既知・後追いリスク / スパイク・局地的反応
    elif pri == "👀 A (保留)":
        if is_spike_flag:
    ...
    # 【パターン3系】C（見送り）: 反応弱・広がり乏しい・追う理由薄い
    else: 
        if is_spike_flag:
