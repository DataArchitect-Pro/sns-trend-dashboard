...
if df.empty:
    ...
    # 確定事実と推定原因を分離
    st.markdown("現在のCSVでは、しきい値を満たすキーワードが見つかりませんでした。<br><span style='color: #555;'>主な原因候補：一般語・ノイズ語が多い / 投稿数が少ない / 出現回数の分散が小さい</span>", unsafe_allow_html=True)
    
    st.write("") # 余白
    col_err_left, col_err_right = st.columns([1, 1], gap="large")
    
    with col_err_left:
        # 動的診断による「最も可能性が高い原因」の提示
        drop_reason = metadata.get('drop_reason', '条件未達')
        st.error(f"**🔍 最も可能性が高い原因**\n\n{drop_reason}")
        
        # 行動の優先順位を整理し、具体的な数値を提示
        suggested_min_freq = max(1, min_freq - 1)
        st.info(f"""
        **💡 推奨されるアクション**
        1. 左サイドバーから **最低出現回数を下げる** （まずは {min_freq} → **{suggested_min_freq}** に変更）
        2. より投稿数が多く、具体的な本文を含むCSVに差し替える
        3. 必要に応じて「ノイズ・スパム判定語を表示する」をONにして除外内容を確認する
        """)
    ...
    with col_err_right:
        ...
        <div style="background-color: #f9f9fc; padding: 20px; border-radius: 8px; border: 1px solid #e0e0e0;">
            <strong style="color: #333; font-size: 1.1em;">📊 現在の解析状況</strong><br><br>
            ...
            <span style="color: #555;">しきい値を通過した語数：</span> <b style="color: #d32f2f; font-size: 1.1em;">{passed}件</b><br><br>
            <hr style="margin: 12px 0; border: none; border-top: 1px dashed #ccc;">
            <strong style="color: #555; font-size: 0.95em;">⚙️ 現在の設定値</strong><br>
            <span style="color: #666; font-size: 0.9em;">最低出現回数： <b>{min_freq}</b></span><br>
