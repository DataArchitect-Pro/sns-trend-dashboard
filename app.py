...
def enrich_card_data(row):
    ...
    if pri == "🔥 S (最優先)":
        ...
        # Aランクの理由をより条件付き・客観的なものに改善
        if ctype in ["まとめ型", "解説型"]:
            return "監視継続", "現状は注目されているが、先回り余地は限定的"
        else:
            return "監視継続", "局地的な反応はあるが、ポテンシャル(EOS)の上昇待ち"
...
# マップ上のラベル設定（重なりを防ぐため、番号のみを表示し、キーワードは非表示にする）
...
if not top3_ideas.empty:
    for _, row in top3_ideas.iterrows():
        # 行間の余白を広げ、アクションを青リンク風ではなく「薄いバッジ風」に変更して視認性UP
        st.markdown(f"""
        <div style="margin-bottom: 12px;">
            <span style="font-size: 1.1em; font-weight: bold; margin-right: 8px;">{row['Rank_Num']} {row['token']}</span>
            <span style="background-color: #e3f2fd; color: #1976d2; padding: 4px 8px; border-radius: 4px; font-size: 0.85em; font-weight: bold; margin-right: 8px;">{row['action']}</span>
            <span style="color: #666; font-size: 0.85em;">（{row['text_content_type']}）</span><br>
            <span style="color: #555; font-size: 0.9em; margin-left: 32px; display: inline-block; margin-top: 4px;">{row['reason']}</span>
        </div>
        """, unsafe_allow_html=True)
else:
...
# タイトルとの間に少し余白を持たせ、文字色を少しだけ濃く
st.markdown("<div style='color: #333; font-weight: bold; font-size: 0.95em; margin-top: 8px; margin-bottom: 16px;'>左上=先回り候補 / 右上=本命 / 右下=後追い / 左下=見送り</div>", unsafe_allow_html=True)
...
# マップ上は番号(①②③)のみ表示され、キーワード名はホバーで確認できるため絶対に詰まらない
fig.update_traces(textposition='top right', textfont_size=16, textfont_color="#d32f2f")
...
# ラベルを短縮し、初期状態は閉じておく
with st.expander("💡 投稿型の説明", expanded=False):
    st.markdown("""
