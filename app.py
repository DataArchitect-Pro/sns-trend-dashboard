...
# 推奨アクションと型の分離、優先度に連動した理由の明確化
def enrich_card_data(row):
    ctype = row['text_content_type']
    pri = row['priority']
    
    if pri == "🔥 S (最優先)":
        if ctype == "先読み型": return "今すぐ仕込み投稿", "成長率が高く、競争がまだ浅いため"
        elif ctype == "解説型": return "今すぐ投稿", "話題性が急上昇しており、検索需要が高いため"
        elif ctype == "比較型": return "今すぐ投稿", "関連語との結びつきが強く、違いへの関心が高いため"
        elif ctype == "まとめ型": return "今すぐ投稿", "圧倒的な話題量を持ち、反応が多いため"
        else: return "今すぐ投稿", "話題量・ポテンシャル共に高水準なため"
        
    elif pri == "👀 A (監視・次点)":
        if ctype in ["まとめ型", "解説型"]:
            return "監視継続", "注目度は高いが、先行優位は限定的なため"
        else:
            return "監視継続", "局地的な反応はあるが、仕込みより様子見が妥当なため"
...
# マップ上のラベル設定（TOP3のみ番号を表示し、その他は空にしてホバー中心にする）
df_display['plot_label'] = df_display.apply(lambda r: f"{r['Rank_Num']} {r['token']}" if r['Rank_Num'] else "", axis=1)

# ==========================================
# 7. UI 描画 (結果画面)
# ==========================================
# 通知は非常に薄く1行のみで主張を抑える
st.markdown("<div style='color: #888; font-size: 0.85em; margin-bottom: 16px;'>分析完了：注目テーマと投稿企画案を確認できます。</div>", unsafe_allow_html=True)
...
if not top3_ideas.empty:
    for _, row in top3_ideas.iterrows():
        # リッチな1行リスト（キーワード・アクション・型・理由）
        st.markdown(f"""
        **{row['Rank_Num']} {row['token']}** — <span style='color: #1976d2; font-weight: bold;'>{row['action']}</span>（{row['text_content_type']}）<br>
        <span style='color: #555; font-size: 0.9em; margin-left: 28px;'>{row['reason']}</span>
        """, unsafe_allow_html=True)
else:
...
# 少し濃い色で1行のみの説明
st.markdown("<div style='color: #333; font-weight: bold; font-size: 0.9em;'>左上=先回り候補 / 右上=本命 / 右下=後追い / 左下=見送り</div>", unsafe_allow_html=True)
...
# テキストを点の真上に配置し、TOP3以外はホバーでのみ表示させて重なりを防ぐ
fig.update_traces(textposition='top center', textfont_size=15, textfont_color="#d32f2f")
...
# 列順を行動順（優先度・キーワード・アクション・型・理由・スコア）に固定
view_cols = {
    'priority': '優先度',
    'token': 'キーワード', 
    'action': '推奨アクション',
...
with st.expander("💡 各「投稿型」の意味と狙い", expanded=False):
    st.markdown("""
